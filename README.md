...
- **Day boundary**: puzzles roll at **00:00 UTC**, the same instant the board seals
  and the pot pays
- **Test**: `npm test` — 82 checks across all three modes, endless, the
  archive, the board and the design contract, in real headless Edge.
  `tools/run-test.mjs` brings up the static server and the browser the suite
  expects, runs it, and tears both down. Any console error is a failure.
- **Look at it**: `node tools/shot.mjs <name> [w] [h] [actions…]` renders the
  page and writes `output/<name>.png`. Actions: `full` `live` `seed` `reveal`
  `route=<hash>` `click=<sel>` `type=<text>` `page=<file>`. Routes are passed
  **without** a leading `#` — Git Bash rewrites those into Windows paths.
...
- **The world**: the landscape is hand-authored inline SVG, generated by
  `node tools/build-scene.mjs` and spliced into `index.html` between the
  `<!--scene-->` markers. Re-run it after editing the generator. Its profile is
  a valley on purpose — the land rises at the edges and dips through the middle,
  so the content column and the footer sit over paper rather than over green.
  The two mascots are transparent cut-outs from `img/art/`, built by
  `tools/build-art.js` from `tools/art-sources.json`. The pre-rendered
  `img/memedle-mascot-horizon-v2.webp` backdrop belonged to the retired pixel
  world and is no longer loaded by the page.
- **Motion and sound**: `vendor/gsap.min.js` is vendored, not a CDN link.
  `motion.js` owns the idle life in the world layer and the staggered entrance;
  everything a finger touches stays on a CSS transition. `sfx.js` synthesises
  every cue from oscillators — no audio files, and silent until the player opts
  in. Append `?still` to freeze every timeline for a reproducible screenshot.
